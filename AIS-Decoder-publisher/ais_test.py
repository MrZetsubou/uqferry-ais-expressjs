...
if __name__ == "__main__":
    import sys, getopt, mosquitto, telnetlib, getpass
    ...
    try:
        ...
        raise SystemExit, 1
    client = mosquitto.Mosquitto("Malcolm_AIS")
    client.connect("winter.ceit.uq.edu.au")
    tn = telnetlib.Telnet("5.9.207.224", "6466") 
    dsv = False
    dump = False
    histogram = False
    json = False
    ...
    try:
        for (raw, parsed, bogon) in parse_ais_messages(sys.stdin, scaled, skiperr, verbose):
            msgtype = parsed[0][1]
            client.publish("/uq/ferry/raw1", raw, 0)
            tn.write(raw)
            ...
            if not bogon:
                if json:   
                    ...
                    ais_message = "{" + ",".join(map(lambda x: '"' + x[0].name + '":' + str(x[1]), parsed)) + ", " + '"TimeStamp":' + '"' + strftime("%a, %d %b %Y %X +0800", localtime()) + '"' + "}"
                    print ais_message
                    client.publish("/uq/ferry/json1", ais_message, 0, 0)
                elif dsv:
